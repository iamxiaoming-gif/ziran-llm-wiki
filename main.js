...
`,v),P=h.indexOf(`
`,k+1);h=h.substring(0,P+1)+y+`
`+h.substring(P+1)}}return await this.app.vault.modify(n,h),{success:!0,content:`\u7AE0\u8282\u300C${e.section}\u300D\u5DF2\u66F4\u65B0: ${t}
...
`+_+`
`+i.substring(m);await this.app.vault.modify(s,w);let v=w.match(/知识点总数：(\d+)个/);if(v){let k=parseInt(v[1])+1,P=w.replace(`\u77E5\u8BC6\u70B9\u603B\u6570\uFF1A${v[1]}\u4E2A`,`\u77E5\u8BC6\u70B9\u603B\u6570\uFF1A${k}\u4E2A`);await this.app.vault.modify(s,P)}}if(e.keywords){let u=`${t}/20-\u77E5\u8BC6\u7D22\u5F15/\u5173\u952E\u8BCD\u7D22\u5F15.md`,d=this.app.vault.getAbstractFileByPath((0,a.normalizePath)(u));if(d&&d instanceof a.TFile){let m=await this.app.vault.read(d),r=e.keywords.split(",").map(h=>`| ${h.trim()} | [[${e.entry_name}]] | 1 |`).join(`
...
\u2022 **'Lint'** - \u6267\u884C\u6574\u7406\u68C0\u67E5
\u2022 **'\u521B\u5EFA\u77E5\u8BC6\u70B9'** - \u65B0\u5EFA\u77E5\u8BC6\u70B9\u9875\u9762
\u2022 **'\u77E5\u8BC6\u5E93\u72B6\u6001'** - \u67E5\u770B\u77E5\u8BC6\u5E93\u6982\u51B5`))}async sendMessage(){let t=this.inputEl.value.trim();if(!(!t||this.isProcessing)){this.inputEl.value="",this.isProcessing=!0,this.sendBtn.addClass("llm-wiki-hidden"),this.stopBtn.removeClass("llm-wiki-hidden"),this.addUserMessage(t),this.addAssistantMessage("");try{let n=this.plugin.agentCore;if(!n){this.updateAssistantMessage("\u274C Agent \u672A\u521D\u59CB\u5316\uFF0C\u8BF7\u68C0\u67E5 API Key \u8BBE\u7F6E\u3002");return}let s=async i=>{this.finalizeAssistantMessage(),this.autoLog(t,i),await this.saveChatHistory()};this.plugin.settings.streamMode?await n.chatStream(t,{onToken:i=>this.appendToken(i),onToolCall:(i,o)=>this.showToolCall(i,o),onToolResult:(i,o)=>this.showToolResult(i,o),onComplete:s,onError:i=>{this.updateAssistantMessage(`\u274C ${i}`),this.finalizeAssistantMessage()}}):await n.chatNonStream(t,{onToken:i=>this.appendToken(i),onToolCall:(i,o)=>this.showToolCall(i,o),onToolResult:(i,o)=>this.showToolResult(i,o),onComplete:s,onError:i=>{this.updateAssistantMessage(`\u274C ${i}`),this.finalizeAssistantMessage()}})}catch(n){this.updateAssistantMessage(`\u274C \u53D1\u751F\u9519\u8BEF: ${n instanceof Error?n.message:String(n)}`)}finally{this.isProcessing=!1,this.sendBtn.removeClass("llm-wiki-hidden"),this.stopBtn.addClass("llm-wiki-hidden"),this.toolCardEl=null,this.inputEl.focus()}}}stopGeneration(){var t;(t=this.plugin.agentCore)==null||t.abort(),this.isProcessing=!1,this.sendBtn.removeClass("llm-wiki-hidden"),this.stopBtn.addClass("llm-wiki-hidden")}async addUserMessage(t){let n=this.messagesEl.createEl("div",{cls:"llm-wiki-message llm-wiki-user-message"});n.createEl("div",{cls:"llm-wiki-message-sender",text:"\u4F60"});let s=n.createEl("div",{cls:"llm-wiki-message-content"});await C.MarkdownRenderer.render(this.app,t,s,"",this.plugin),this.scrollToBottom()}addSystemMessage(t){let s=this.messagesEl.createEl("div",{cls:"llm-wiki-message llm-wiki-system-message"}).createEl("div",{cls:"llm-wiki-message-content"});C.MarkdownRenderer.render(this.app,t,s,"",this.plugin)}addAssistantMessage(t){let n=this.messagesEl.createEl("div",{cls:"llm-wiki-message llm-wiki-assistant-message"});n.createEl("div",{cls:"llm-wiki-message-sender",text:"Agent"}),this.currentAssistantEl=n.createEl("div",{cls:"llm-wiki-message-content"}),this.currentContent=t,this.tokenBuffer=""}updateAssistantMessage(t){this.currentAssistantEl&&(this.currentContent=t,this.currentAssistantEl.empty(),C.MarkdownRenderer.render(this.app,t,this.currentAssistantEl,"",this.plugin),this.scrollToBottom())}appendToken(t){this.currentContent+=t,this.tokenBuffer+=t,this.renderTimer||(this.renderTimer=window.setInterval(()=>this.flushTokenBuffer(),50)),this.scrollToBottom()}flushTokenBuffer(){this.renderTimer&&(window.clearInterval(this.renderTimer),this.renderTimer=null),this.tokenBuffer&&this.currentAssistantEl&&(this.currentAssistantEl.empty(),C.MarkdownRenderer.render(this.app,this.currentContent,this.currentAssistantEl,"",this.plugin),this.tokenBuffer="")}finalizeAssistantMessage(){this.flushTokenBuffer(),this.currentAssistantEl=null,this.currentContent=""}showToolCall(t,n){let s=JSON.stringify(n,null,2);if(this.currentAssistantEl){let i=this.currentAssistantEl.createEl("div",{cls:"llm-wiki-tool-card llm-wiki-tool-running"});i.createEl("div",{cls:"llm-wiki-tool-name",text:`\u{1F7E1} \u8C03\u7528\u5DE5\u5177: ${t}`});let o=i.createEl("pre",{cls:"llm-wiki-tool-args"});o.textContent=s,this.toolCardEl=i}this.scrollToBottom()}showToolResult(t,n){if(this.toolCardEl){let s=n.success?`\u{1F7E2} ${t} \u5B8C\u6210`:`\u{1F534} ${t} \u5931\u8D25`,i=this.toolCardEl.querySelector(".llm-wiki-tool-name");if(i&&(i.textContent=s),this.toolCardEl.removeClass("llm-wiki-tool-running"),this.toolCardEl.addClass(n.success?"llm-wiki-tool-success":"llm-wiki-tool-error"),n.content){let o=this.toolCardEl.createEl("div",{cls:"llm-wiki-tool-result"});o.textContent=n.content.length>300?n.content.substring(0,300)+"...":n.content}this.toolCardEl=null}this.scrollToBottom()}scrollToBottom(){this.messagesEl.scrollTop=this.messagesEl.scrollHeight}async autoLog(t,n){if(!(!this.plugin.settings.autoLog||!this.plugin.memoryService))try{let s=t.length>50?t.substring(0,50)+"...":t,i=n.length>200?n.substring(0,200)+"...":n;await this.plugin.memoryService.writeLog(s,`\u7528\u6237: ${t}`,i)}catch(s){}}async saveChatHistory(){var t;try{let n=(t=this.plugin.agentCore)==null?void 0:t.getHistory();if(!n||n.length===0)return;let s={messages:n,savedAt:new Date().toISOString()},i=(0,C.normalizePath)(`${this.plugin.settings.memoryFolder}/\u5BF9\u8BDD\u5386\u53F2.json`),o=this.app.vault.getAbstractFileByPath(i);o&&o instanceof C.TFile?await this.app.vault.modify(o,JSON.stringify(s,null,2)):await this.app.vault.create(i,JSON.stringify(s,null,2))}catch(n){}}async loadChatHistory(){var t;try{let n=(0,C.normalizePath)(`${this.plugin.settings.memoryFolder}/\u5BF9\u8BDD\u5386\u53F2.json`),s=this.app.vault.getAbstractFileByPath(n);if(!s||!(s instanceof C.TFile)){this.addSystemMessage(`\u6B22\u8FCE\u4F7F\u7528 LLM Wiki \u77E5\u8BC6\u5E93\u52A9\u624B\uFF01

\u8BF7\u5148\u5B8C\u6210\u4EE5\u4E0B\u8BBE\u7F6E\uFF1A
1. \u5728\u8BBE\u7F6E\u4E2D\u586B\u5199 API Key\uFF08\u652F\u6301 OpenAI / DeepSeek / \u7845\u57FA\u6D41\u52A8\u7B49\uFF09
2. \u8BBE\u7F6E\u77E5\u8BC6\u5E93\u8DEF\u5F84

\u7136\u540E\u5C31\u53EF\u4EE5\u5F00\u59CB\u5BF9\u8BDD\u4E86\uFF01`);return}let i=await this.app.vault.read(s),o=JSON.parse(i);(t=this.plugin.agentCore)==null||t.setHistory(o.messages||[]);for(let c of o.messages||[])c.role==="user"?this.addUserMessage(c.content):c.role==="assistant"&&!c.tool_calls&&(this.addAssistantMessage(c.content),this.finalizeAssistantMessage())}catch(n){this.addSystemMessage("\u6B22\u8FCE\u56DE\u6765\uFF01\u4E0A\u6B21\u7684\u5BF9\u8BDD\u5386\u53F2\u52A0\u8F7D\u5931\u8D25\uFF0C\u5DF2\u5F00\u542F\u65B0\u5BF9\u8BDD\u3002")}}};var x=require("obsidian"),D=class{constructor(e,t){this.app=e,this.settings=t}updateSettings(e){this.settings=e}async loadSkillContent(){try{let e=(0,x.normalizePath)(`${this.settings.skillFolderPath}/SKILL.md`),t=this.app.vault.getAbstractFileByPath(e);if(t&&t instanceof x.TFile)return await this.app.vault.read(t)}catch(e){}return""}async loadReferencesContent(){try{let e=[],t=(0,x.normalizePath)(`${this.settings.skillFolderPath}/references`),n=this.app.vault.getAbstractFileByPath(t);if(n&&n instanceof x.TFolder){for(let s of n.children)if(s instanceof x.TFile&&s.extension==="md"){let i=await this.app.vault.read(s);e.push(`### ${s.name}

${i}`)}}return e.join(`

---

`)}catch(e){}return""}async loadMemoryContext(){let e=[],t=(0,x.normalizePath)(`${this.settings.memoryFolder}/\u957F\u671F\u8BB0\u5FC6.md`),n=this.app.vault.getAbstractFileByPath(t);n&&n instanceof x.TFile&&e.push(await this.app.vault.read(n));let s=(0,x.normalizePath)(`${this.settings.memoryFolder}/\u7528\u6237\u504F\u597D.md`),i=this.app.vault.getAbstractFileByPath(s);return i&&i instanceof x.TFile&&e.push(await this.app.vault.read(i)),e.join(`

---

`)}async writeLog(e,t,n){let s=new Date().toISOString().split("T")[0],i=(0,x.normalizePath)(`${this.settings.memoryFolder}/\u65E5\u5FD7/${s}.md`),o=`## ${new Date().toLocaleTimeString("zh-CN",{hour:"2-digit",minute:"2-digit"})} | ${e}
...
---
`,c=this.app.vault.getAbstractFileByPath(i);if(c&&c instanceof x.TFile){let l=await this.app.vault.read(c);await this.app.vault.modify(c,l+o)}else await this.ensureFolder(`${this.settings.memoryFolder}/\u65E5\u5FD7`),await this.app.vault.create(i,`# \u5DE5\u4F5C\u65E5\u5FD7 ${s}

${o}`)}async ensureFolder(e){if(!e)return;let t=(0,x.normalizePath)(e).split("/"),n="";for(let s of t)n=n?`${n}/${s}`:s,this.app.vault.getAbstractFileByPath(n)||await this.app.vault.createFolder(n)}};var R=class{constructor(){this.summaryCache=new Map;this.maxTokens=8e3}setMaxTokens(e){this.maxTokens=e}estimateTokens(e){return Math.ceil(e.length/2)}async compressHistory(e,t){let n=e.map(r=>r.content||"").join(`
...
`);return{originalTokens:this.estimateTokens(t),compressedTokens:Math.ceil(t.length/3),turns:e.filter(n=>n.role==="user").length}}};var q=class extends N.Plugin{async onload(){await this.loadSettings(),this.addSettingTab(new B(this.app,this)),this.toolRegistry=new I(this.app,this.settings),this.memoryService=new D(this.app,this.settings),this.contextManager=new R,await this.initAgent(),this.registerView(M,t=>new j(t,this)),this.addCommand({id:"open-llm-wiki-chat",name:"\u6253\u5F00 LLM Wiki \u77E5\u8BC6\u5E93\u52A9\u624B",callback:()=>this.activateChatView()}),this.addRibbonIcon("message-square","LLM Wiki \u77E5\u8BC6\u5E93\u52A9\u624B",()=>{this.activateChatView()}),this.applyTheme(),this.app.workspace.onLayoutReady(()=>{this.activateChatView()})}onunload(){}async initAgent(){let[t,n,s]=await Promise.all([this.memoryService.loadSkillContent(),this.memoryService.loadReferencesContent(),this.memoryService.loadMemoryContext()]);this.agentCore=new O(this.settings,this.toolRegistry),this.agentCore.init(t,n,s)}async activateChatView(){let{workspace:t}=this.app,n=t.getLeavesOfType(M)[0];if(!n){let s=t.getRightLeaf(!1);s&&(await s.setViewState({type:M,active:!0}),n=t.getLeavesOfType(M)[0])}n&&t.revealLeaf(n)}applyTheme(){let t=N.activeDocument;t.body.classList.remove("llm-wiki-theme-dark-blue","llm-wiki-theme-warm-light","llm-wiki-theme-obsidian-red","llm-wiki-theme-lavender","llm-wiki-theme-forest-green"),t.body.classList.add(`llm-wiki-theme-${this.settings.theme}`)}async loadSettings(){this.settings=Object.assign({},H,await this.loadData())}async saveSettings(){await this.saveData(this.settings),await this.initAgent(),this.memoryService.updateSettings(this.settings)}};
