...
### `,k+1),L=P===-1?g.indexOf(`
---`,k):P,F=g.substring(k,L),S=F.match(/\((\d+)个\)/),T=S?parseInt(S[1]):0,E=F.replace(`(${T}\u4E2A)`,`(${T+1}\u4E2A)`).replace(/（暂无知识点）/,""),A=`- [[${e.title}]] ${o} - ${e.entry_description||"\u5F85\u8865\u5145"}`,N=g.substring(0,k)+E.trimEnd()+`
`+A+`
`+g.substring(L);await this.app.vault.modify(p,N);let W=N.match(/知识点总数：(\d+)个/);if(W){let J=parseInt(W[1])+1;await this.app.vault.modify(p,N.replace(`\u77E5\u8BC6\u70B9\u603B\u6570\uFF1A${W[1]}\u4E2A`,`\u77E5\u8BC6\u70B9\u603B\u6570\uFF1A${J}\u4E2A`))}}if(e.keywords){let k=`${s}/20-\u77E5\u8BC6\u7D22\u5F15/\u5173\u952E\u8BCD\u7D22\u5F15.md`,P=this.app.vault.getAbstractFileByPath((0,a.normalizePath)(k));if(P&&P instanceof a.TFile){let L=await this.app.vault.read(P),F=e.keywords.split(",").map(S=>`| ${S.trim()} | [[${e.title}]] | 1 |`).join(`
