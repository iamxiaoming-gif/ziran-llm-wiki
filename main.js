...
\u2514\u2500\u2500 AGENTS.md
\`\`\`
`;try{let m=(0,h.normalizePath)(`${this.settings.skillFolderPath}/references/AGENTS-template.md`),v=this.app.vault.getAbstractFileByPath(m);v&&v instanceof h.TFile?g=(await this.app.vault.read(v)).replace(/\[专题名称\]/g,t.topic_name).replace(/YYYY-MM-DD/g,new Date().toISOString().split("T")[0]).replace(/\[方括号\]/g,""):z["AGENTS-template.md"]&&(g=z["AGENTS-template.md"].replace(/\[专题名称\]/g,t.topic_name).replace(/YYYY-MM-DD/g,new Date().toISOString().split("T")[0]).replace(/\[方括号\]/g,""))}catch(m){z["AGENTS-template.md"]&&(g=z["AGENTS-template.md"].replace(/\[专题名称\]/g,t.topic_name).replace(/YYYY-MM-DD/g,new Date().toISOString().split("T")[0]).replace(/\[方括号\]/g,""))}await this.createFileOnly(`${e}/AGENTS.md`,g);let b=n.length>0?`\u77E5\u8BC6\u70B9\u5E93\u5206\u7C7B\uFF1A${n.join("\u3001")}\uFF08${n.length} \u4E2A\uFF09`:"\u77E5\u8BC6\u70B9\u5E93\u6682\u672A\u521B\u5EFA\u5206\u7C7B \u2014 \u8BF7\u5728\u6444\u53D6\u539F\u59CB\u8D44\u6599\u540E\uFF0C\u6839\u636E\u8D44\u6599\u5185\u5BB9\u63A8\u8350\u5408\u9002\u7684\u5206\u7C7B\uFF0C\u7136\u540E\u7528 create_vault_folder \u5728 10-\u77E5\u8BC6\u70B9\u5E93/ \u4E0B\u521B\u5EFA\u5206\u7C7B\u6587\u4EF6\u5939";return{success:!0,content:`\u77E5\u8BC6\u5E93 "${t.topic_name}" \u5DF2\u521D\u59CB\u5316\u5B8C\u6210\uFF01
...
`)}var ht=class extends gt.Plugin{constructor(){super(...arguments);this.batchStatusBarEl=null;this.unsubscribeBatchStatus=null;this.currentSystemPrompt=""}async onload(){await this.loadSettings(),this.ingestionService=new O(this.app,this.settings),this.toolRegistry=new tt(this.app,this.settings,this.ingestionService),this.memoryService=new st(this.app,this.settings),this.contextManager=new nt,this.historySanitizer=new it,this.transcriptionService=new rt(this.settings),this.knowledgeEvidenceService=new at(this.app,this.settings),this.feynmanEvaluationService=new ct(this.settings),this.feynmanSessionService=new lt(this.app,this.settings),this.addSettingTab(new Z(this.app,this)),await this.initAgent();let t=e=>{let n=new J(this.settings,e,this.contextManager);return n.init(this.currentSystemPrompt),n};this.backgroundIngestionService=new dt(this.app,this.settings,this.toolRegistry,t,this.ingestionService),this.toolRegistry.setBackgroundIngestionService(this.backgroundIngestionService),this.backgroundIngestionService.init().catch(e=>{console.error("\u6062\u590D\u6444\u53D6\u6279\u6B21\u72B6\u6001\u5931\u8D25:",e)}),this.registerView(V,e=>new et(e,this)),this.registerView(Y,e=>new ut(e,this)),this.addCommand({id:"open-llm-wiki-chat",name:"\u6253\u5F00 LLM Wiki \u77E5\u8BC6\u5E93\u52A9\u624B",callback:()=>this.activateChatView()}),this.addCommand({id:"open-feynman-learning",name:"\u6253\u5F00\u8D39\u66FC\u5B66\u4E60\u6559\u7EC3",callback:()=>this.activateFeynmanView()}),this.addRibbonIcon("message-square","LLM Wiki \u77E5\u8BC6\u5E93\u52A9\u624B",()=>{this.activateChatView()}),this.addRibbonIcon("brain","\u8D39\u66FC\u5B66\u4E60\u6559\u7EC3",()=>{this.activateFeynmanView()}),this.initBatchStatusBar(),this.applyTheme(),this.app.workspace.onLayoutReady(()=>{this.activateChatView()})}onunload(){var t;(t=this.unsubscribeBatchStatus)==null||t.call(this),this.unsubscribeBatchStatus=null}initBatchStatusBar(){this.batchStatusBarEl=this.addStatusBarItem(),this.batchStatusBarEl.addClass("llm-wiki-batch-status-bar"),this.batchStatusBarEl.addClass("llm-wiki-hidden"),this.batchStatusBarEl.setAttribute("aria-label","\u540E\u53F0\u6444\u53D6\u8FDB\u5EA6"),this.batchStatusBarEl.addEventListener("click",()=>void this.activateChatView()),this.unsubscribeBatchStatus=this.backgroundIngestionService.subscribe(t=>this.renderBatchStatusBar(t)),this.backgroundIngestionService.getSnapshot().then(t=>this.renderBatchStatusBar(t)).catch(()=>{})}renderBatchStatusBar(t){if(!this.batchStatusBarEl)return;let e=Object.values(t.totals).reduce((c,l)=>c+l,0),n=t.status==="completed"||t.status==="completed_with_errors";if(e===0){this.batchStatusBarEl.addClass("llm-wiki-hidden");return}if(n){let c=t.totals.completed+t.totals.skipped+t.totals.failed;this.batchStatusBarEl.removeClass("llm-wiki-hidden"),this.batchStatusBarEl.setText(`\u2705 \u6444\u53D6\u5B8C\u6210 ${c}/${e}`),this.batchStatusBarEl.setAttribute("aria-label","\u672C\u6279\u6444\u53D6\u5DF2\u5B8C\u6210 \u2014 \u70B9\u51FB\u6253\u5F00\u52A9\u624B"),window.setTimeout(()=>{var l;(l=this.batchStatusBarEl)==null||l.addClass("llm-wiki-hidden")},5e3);return}this.batchStatusBarEl.removeClass("llm-wiki-hidden");let i=t.totals.completed+t.totals.skipped+t.totals.failed,r=e>0?Math.min(100,Math.round(i/e*100)):0,a=t.status==="active"?"\u26A1":t.status==="stopping"?"\u23F9":t.status==="paused"?"\u23F8":"\u{1F4E6}",o=t.status==="stopping"?"\u505C\u6B62\u4E2D":"\u6444\u53D6";this.batchStatusBarEl.setText(`${a} ${o} ${i}/${e} \xB7 ${r}%`),this.batchStatusBarEl.setAttribute("aria-label",`${t.message||"\u540E\u53F0\u6444\u53D6\u4E2D"} \u2014 \u70B9\u51FB\u6253\u5F00\u52A9\u624B`)}async initAgent(){let[t,e]=await Promise.all([this.memoryService.loadAgentsContext(this.settings.knowledgeBasePath),this.memoryService.loadMemoryContext()]);this.currentSystemPrompt=Lt(this.settings,t,e),this.agentCore?(this.agentCore.updateSettings(this.settings),this.agentCore.updateSystemContext(this.currentSystemPrompt)):(this.agentCore=new J(this.settings,this.toolRegistry,this.contextManager),this.agentCore.init(this.currentSystemPrompt))}async activateChatView(){let{workspace:t}=this.app,e=t.getLeavesOfType(V)[0];if(!e){let n=t.getRightLeaf(!1);n&&(await n.setViewState({type:V,active:!0}),e=t.getLeavesOfType(V)[0])}e&&t.revealLeaf(e)}async activateFeynmanView(){let{workspace:t}=this.app,e=t.getLeavesOfType(Y)[0];if(!e){let n=t.getRightLeaf(!1);n&&(await n.setViewState({type:Y,active:!0}),e=t.getLeavesOfType(Y)[0])}e&&t.revealLeaf(e)}applyTheme(){try{let t=this.app.workspace.getActiveViewOfType(gt.ItemView),e=t?t.containerEl.ownerDocument:window.document;e.body.classList.remove(...wt),e.body.classList.add(`llm-wiki-theme-${this.settings.theme}`)}catch(t){window.document.body.classList.remove(...wt),window.document.body.classList.add(`llm-wiki-theme-${this.settings.theme}`)}}async loadSettings(){let t=await this.loadData();this.settings=Object.assign({},B,t),this.settings=U(this.settings)}async saveSettings(){this.settings=U(this.settings),await this.saveData(this.settings),this.memoryService.updateSettings(this.settings),this.toolRegistry.updateSettings(this.settings),this.transcriptionService.updateSettings(this.settings),this.knowledgeEvidenceService.updateSettings(this.settings),this.feynmanEvaluationService.updateSettings(this.settings),this.feynmanSessionService.updateSettings(this.settings),this.backgroundIngestionService.updateSettings(this.settings),await this.initAgent(),this.applyTheme()}};
